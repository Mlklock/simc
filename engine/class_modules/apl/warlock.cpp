...
void demonology( player_t* p )
{
  ...
  precombat->add_action( "variable,name=trinket_1_exclude,value=trinket.2.is.ruby_whelp_shell" );
  ...
  default_->add_action( "cooldown.demonic_strength.remains|!talent.demonic_strength)&(!raid_event.adds.exists|raid_event.adds.exists&raid_event.adds.remains>6)" );
